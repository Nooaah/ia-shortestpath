...
class Labyrinthe {
	...
	public Labyrinthe(int ttailleX, int ttailleY, long sseedCreationLaby, long sseedPlacementBonus) {
		if (ttailleX < 7)
			throw new java.lang.RuntimeException(
					"Constructeur Labyrinthe prenant taille en param\n-->tailleX < 0 ou trop faible");
		if (ttailleY < 7)
			throw new java.lang.RuntimeException(
					"Constructeur Labyrinthe prenant taille en param\n-->tailleY < 0ou trop faible");
		if (ttailleX % 2 == 0)
			throw new java.lang.RuntimeException(
					"Classe Labyrinthe constructeur 2 entiers\n-->tailleX ne doit pas être pair");
		if (ttailleY % 2 == 0)
			throw new java.lang.RuntimeException(
	...
	// Cette méthode ne doit être appelée qu'après avoir créé un labyrinthe de base
	// par le constructeur prenant 2 entiers en paramètres - donc un labyrinthe
	// composé seulement de dunes mais avec des zones de sable toutes les 2 cases.
	private void genereLabyrinthe() {
		...
		// On cré un tableau temporaire contenant les codes qui seront fusionnés par la
		// suite.
		// voir wikipédia
		// On initialise les codes à 0 pour les cases contenant des dunes, à -1 pour les
		// autres
		...
		// On modifie les -1 par un indice permettant de numéroter toutes les cases
		// n'étant pas des dunes.
		int indice = 0;
		for (int i = 0; i < tableauFlag.length; i++)
			if (tableauFlag[i] == -1)
		...
		while (!finGeneration(tableauFlag)) {
			...
			// Avant de fusionner, on vérifie bien qu'on essaie de casser un mur qu'on peut
			// casser
		...
		for (int i = 1; i < tailleX - 1; i++)
			for (int j = 1; j < tailleY - 1; j++)
				if (getXY(i, j).getType() == Case.DUNE)
					nbDunes++;
		// System.out.println("Il y a "+nbDunes+" murs sur "+(tailleX-2)*(tailleY-2)+"
		// cases soit un pourcentage de :
		// "+(nbDunes*1.0)/((tailleX-2)*(tailleY-2))*100.0);
	}
	...
	private void ajouterJoueur(int posJoueurX, int posJoueurY, String nomJoueur) {
		...
		// System.out.println("Joueur "+nomJoueur+" a bien été ajouté : il porte le
		// numéro "+indice);
	}
	...
	public void faireBougerJoueur(int nbJoueur, char direction) {
		...
		if (getXY(listeJoueur.get(nbJoueur).getPosX() + deltaX, listeJoueur.get(nbJoueur).getPosY() + deltaY)
				.getType() == Case.DUNE)
	...
	private void ramasseEtMAJ(int indiceJoueur) {
		...
		case Case.BIERE:
			...
			break;
		case Case.MOULE:
			listeJoueur.get(indiceJoueur).ajouterPoints(
					getXY(listeJoueur.get(indiceJoueur).getPosX(), listeJoueur.get(indiceJoueur).getPosY())
							.getPointRapporte());
				...
				if (listeJoueur.get(indiceJoueur).getNbFrite() > 0) {
					...
					if (eltCmd.length > 2) {
						...
						if (directionValide(direction)) {
							int joueurPosX = listeJoueur.get(indiceJoueur).getPosX(),
									joueurPosY = listeJoueur.get(indiceJoueur).getPosY();
							...
							if (joueurPosX < 0 || joueurPosX >= tailleX || joueurPosY < 0 || joueurPosY >= tailleY)
								...
								if (getXY(joueurPosX, joueurPosY).getType() != Case.DUNE) {
									...
									System.out.println(
											"Impossible de se déplacer sur une dune. Vous ne pouvez sauter qu'au dessus d'une simple dune. On va tout de même tenter un déplacement simple.");
