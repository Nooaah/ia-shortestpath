...
class ServeurEpreuve1 {
	...
	public static void usage() {
		...
		System.out.println(
				"-screen <String> : permet de prendre des screenshots à chaque mouvement de d'enregistrer les images dans le répertoire passé en paramètre");
		System.exit(0);
	}
	...
	private static String getNomFichierImageBloc(int x, int y) {
		...
		// oxo
		// ooo
		if (la.getXY(x, y - 1).getType() != Case.DUNE && la.getXY(x - 1, y).getType() != Case.DUNE
				&& la.getXY(x + 1, y).getType() != Case.DUNE && la.getXY(x, y + 1).getType() != Case.DUNE)
			return "bloc14.png";
		// ooo
		// oxo
		// oxo
		...
		// oxo
		// if(la.getXY(x,y-1).getType()==Case.DUNE &&
		// la.getXY(x-1,y).getType()==Case.DUNE && la.getXY(x+1,y).getType()==Case.DUNE
		// && la.getXY(x,y+1).getType()==Case.DUNE)
		return "bloc29.png";
	}
	...
	public static void affichageLabyrinthe() {
		...
		for (int i = 0; i < tailleX; i++)
			for (int j = 0; j < tailleY; j++)
				f.ajouter(new Texture("./images/" + getNomFichierImageBloc(i, j),
						new Point(i * tailleCase, (tailleY - j - 1) * tailleCase), tailleCase, tailleCase));
		...
		for (int i = 0; i < nbRepetition; i++)
			f.ajouter(new Texture("./images/bordMer2.png", new Point(tailleX * tailleCase, (i * tailleYBordMer)),
					tailleXBordMer, tailleYBordMer));
	...
	public static void miseAJourLabyrinthe() {
		...
		for (int i = 0; i < la.getNbJoueur(); i++) {
			 ...
			 * System.out.println("pos joueur : "+la.getJoueur(i).getPosX()+","+la.getJoueur
			 * (i).getPosY());
			 * System.out.println("pos texture : "+textureJoueur.get(i).getA().getX()+","+
			 * textureJoueur.get(i).getA().getY());
			 * System.out.println("pas de deplacement : "+(la.getJoueur(i).getPosX()*
			 * tailleCase-textureJoueur.get(i).getA().getX()));
			 * 
			 * System.out.println("pas de deplacement : "+(la.getJoueur(i).getPosY()*
			 * tailleCase-textureJoueur.get(i).getA().getX()));
			 */
			...
			if (dernierCoup[i] != 'C')
				textureJoueur.set(i,
						new Texture("./images/joueur" + (i + 1) + "_small_" + dernierCoup[i] + ".png",
								new Point(la.getJoueur(i).getPosX() * tailleCase,
										(tailleY - la.getJoueur(i).getPosY() - 1) * tailleCase),
								tailleCase, tailleCase));
			else
				textureJoueur.set(i,
						new Texture("./images/joueur" + (i + 1) + "_small_N.png",
								new Point(la.getJoueur(i).getPosX() * tailleCase,
										(tailleY - la.getJoueur(i).getPosY() - 1) * tailleCase),
								tailleCase, tailleCase));
			f.ajouter(textureJoueur.get(i));

			// textureJoueur.get(i).setA(new
			// Point(la.getJoueur(i).getPosX()*tailleCase,(tailleY-la.getJoueur(i).getPosY()-1)*tailleCase));
		...
		for (int i = 0; i < scores.size(); i++) {
			...
			nomsJoueur.get(i).setA(new Point((int) ((tailleX + 0.2) * tailleCase + nomsJoueur.get(i).getLargeur() / 2),
					nomsJoueur.get(i).getA().getY()));
		}
	...
	public static void main(String args[]) {
		...
		// -tauxDeDune <int> : modifie le taux de dunes (doit être compris entre 0 et
		// 50)
		...
		// for(int i=0;i<la.getNbJoueur();i++)
		// textureJoueur.add(new Texture("./images/joueur"+(i+1)+"_small_E.png",new
		// Point(la.getJoueur(i).getPosX()*tailleCase,(tailleY-la.getJoueur(i).getPosY()-1)*tailleCase),tailleCase,tailleCase));
		textureJoueur.add(new Texture("./images/joueur1_small_E.png", new Point(la.getJoueur(0).getPosX() * tailleCase,
				(tailleY - la.getJoueur(0).getPosY() - 1) * tailleCase), tailleCase, tailleCase));
		if (nbJoueurs > 1)
			textureJoueur
					.add(new Texture("./images/joueur2_small_O.png", new Point(la.getJoueur(1).getPosX() * tailleCase,
							(tailleY - la.getJoueur(1).getPosY() - 1) * tailleCase), tailleCase, tailleCase));
		if (nbJoueurs > 2)
			textureJoueur
					.add(new Texture("./images/joueur3_small_S.png", new Point(la.getJoueur(2).getPosX() * tailleCase,
							(tailleY - la.getJoueur(2).getPosY() - 1) * tailleCase), tailleCase, tailleCase));
		if (nbJoueurs > 3)
			textureJoueur
					.add(new Texture("./images/joueur4_small_N.png", new Point(la.getJoueur(3).getPosX() * tailleCase,
							(tailleY - la.getJoueur(3).getPosY() - 1) * tailleCase), tailleCase, tailleCase));
		...
		for (int i = 0; i < tailleX; i++)
			for (int j = 0; j < tailleY; j++) {
				if (la.getXY(i, j).getType() == Case.MOULE) {
					textureMoule.add(new Texture("./images/moule_small.png",
							new Point(i * tailleCase, (tailleY - j - 1) * tailleCase), tailleCase, tailleCase));
					texturePoint.add(new Texture("./images/" + la.getXY(i, j).getPointRapporte() + ".png",
							new Point(i * tailleCase, (tailleY - j - 1) * tailleCase), tailleCase, tailleCase));
				}
				if (la.getXY(i, j).getType() == Case.FRITE)
					textureFrite.add(new Texture("./images/frite_small.png",
							new Point(i * tailleCase, (tailleY - j - 1) * tailleCase), tailleCase, tailleCase));
				if (la.getXY(i, j).getType() == Case.BIERE)
					textureBiere.add(new Texture("./images/biere_small.png",
							new Point(i * tailleCase, (tailleY - j - 1) * tailleCase), tailleCase, tailleCase));
			}
		...
		Rectangle chargementExterieur = new Rectangle(Couleur.NOIR,
				new Point(tailleCase, ((tailleY * tailleCase) - tailleCase) / 2),
				new Point((tailleX * tailleCase) - tailleCase, ((tailleY * tailleCase) + tailleCase) / 2), false);
		Rectangle chargementInterieur = new Rectangle(Couleur.ROUGE,
				new Point(tailleCase, ((tailleY * tailleCase) - tailleCase) / 2), 0, tailleCase, true);
		...
		try {
			output.write("Création d'un labyrinthe de " + tailleX + "x" + tailleY + " dont les cases font " + tailleCase
					+ " pixels.\n");
			output.flush();
			output.write(la + "\n");
			output.flush();
			...
			output.flush();
			output.write("Le labyrinthe contient un taux de dune de " + tauxDune + "%, " + nbFrite + " frite(s), "
					+ nbMoule + " moule(s) et " + nbBiere + " biére(s)\n");
			output.flush();
			output.write("Le port du serveur est le " + port + ". Le timeout pour chaque joueur est de " + timeout
					+ " ms. le délai d'attente minimum entre deux tours est de " + delay + " ms.\n");
			output.flush();
		...
		System.out.println("Le labyrinthe de " + tailleX + "x" + tailleY + " contient un taux de dune de " + tauxDune
				+ "%, " + nbFrite + " frite(s), " + nbMoule + " moule(s) et " + nbBiere + " biére(s)");
		...
		while (nbConnecte != nbJoueurs) {
			...
			System.out.println("************************************************************\n*    -->    "
					...
					+ "                          *\n*       Connexion " + nbConnecte + " - adresse : " + hostname + ", "
					+ ip + "*\n************************************************************");
			try {
				output.write("************************************************************\n*    -->    "
						+ new SimpleDateFormat("dd/MM/yyyy - HH:mm:ss", Locale.FRANCE).format(new Date())
			...
			nbConnecte++;
			chargementInterieur.setLargeur(
					(int) ((tailleX * tailleCase - 2 * tailleCase) * ((nbConnecte * 1.0) / (nbJoueurs * 1.0))));
			f.rafraichir();
		}
		...
		for (int i = 0; i < nbJoueurs; i++) {
			...
			try {
				...
				nomsJoueur.add(new Texte(c, msgRecu, new Font("Calibri", Font.TYPE1_FONT, tailleFont), new Point(
						(tailleX + 2) * tailleCase, tailleY * tailleCase - (i + 1) * multiplicateur + addition)));
				nomsJoueur.get(i).setA(new Point((int) ((tailleX + 0.2) * tailleCase),
						tailleY * tailleCase - (i + 1) * multiplicateur + addition));
				scores.add(new Texte(c, "0", new Font("Calibri", Font.TYPE1_FONT, tailleFont),
						new Point((tailleX + 2) * tailleCase, tailleY * tailleCase - (i + 1) * multiplicateur)));
			} catch (IOException e) {
				System.out.println("Impossible de lire le nom du joueur " + i);
				nomsJoueur.add(new Texte(c, "Les blaireaux", new Font("Calibri", Font.TYPE1_FONT, tailleFont),
						new Point((tailleX + 2) * tailleCase,
								tailleY * tailleCase - (i + 1) * multiplicateur + addition)));
				nomsJoueur.get(i).setA(new Point((int) ((tailleX + 0.2) * tailleCase),
						tailleY * tailleCase - (i + 1) * multiplicateur + addition));
				scores.add(new Texte(c, "0", new Font("Calibri", Font.TYPE1_FONT, tailleFont),
		...
		while (nbMoule > 0 && toutLeMondeConnecte && (nbTourMax == -1 || ((tour + 1) < nbTourMax * nbJoueurs))) {
			...
			// System.out.println("temps d'attente :
			// "+(delay-((tpsCourant-tpsDepart)/1000)));
			try {
				Thread.sleep(delay - ((tpsCourant - tpsDepart) / 1000));
			...
			if (msgRecu == null) {
				System.out
						.println("Le joueur " + numJoueur + " s'est surement deconnecté car le message est illisible");
				System.out.println("Le mouvement joué sera C pour le joueur " + numJoueur);
				msgRecu = "C";
			}
			System.out.println(
					getDate() + " - réception du message \"" + msgRecu + "\" de la part du joueur " + numJoueur);
			try {
			...
			/*
			 * for(int i=0;i<nbJoueurs;i++) toutLeMondeConnecte=(toutLeMondeConnecte &&
			 * services[i].isConnected());
			 */

			// Vérifier que les derniers coups joués ne sont pas tous des "C"
			// Dans ce cas, cela voudrait dire que tout le monde est déconnecté, que tout le
			// monde a passé ou un mixte des deux.
		...
		for (int i = 1; i < la.getNbJoueur(); i++) {
			if (la.getJoueur(i).getNbPoints() == la.getJoueur(vainqueurs.get(0)).getNbPoints())
				vainqueurs.add(i);
			else if (la.getJoueur(i).getNbPoints() > la.getJoueur(vainqueurs.get(0)).getNbPoints()) {
				vainqueurs.clear();
				vainqueurs.add(i);
			}
		}
		...
		if (vainqueurs.size() == 1) {
			System.out.println("VAINQUEUR : " + la.getJoueur(vainqueurs.get(0)).getNom() + " avec "
					+ la.getJoueur(vainqueurs.get(0)).getNbPoints() + " points");
			try {
				output.write(getDate() + " -  VAINQUEUR : " + la.getJoueur(vainqueurs.get(0)).getNom() + " avec "
						+ la.getJoueur(vainqueurs.get(0)).getNbPoints() + " points\n");
				output.flush();
			} catch (IOException e) {
				System.out.println("Problème lors de l'écriture dans le fichier " + nomFichier);
			...
			for (int i = 0; i < vainqueurs.size(); i++)
				System.out.println(la.getJoueur(vainqueurs.get(i)).getNom() + " avec "
						+ la.getJoueur(vainqueurs.get(i)).getNbPoints() + " points");
		}
